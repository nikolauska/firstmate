...
- **Restart-proof** - all state lives on disk and in the active session backend (Herdr preferred for new work, tmux as the compatibility fallback, cmux also selectable or auto-detectable, and zellij/orca explicitly selectable); kill the session anytime and the next one reconciles, including confirmed-dead secondmate agents, and carries on.
