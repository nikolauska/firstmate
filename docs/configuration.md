...
When no explicit per-task backend, `FM_BACKEND`, or `config/backend` selection exists, Firstmate passes `--backend herdr` for new work and uses tmux only for a concrete Herdr issue.
Firstmate never overrides an explicit backend selection to apply that preference.
...
Task meta records `backend=` only for a non-tmux backend; an absent `backend=` means `tmux`, preserving existing compatibility-path meta files.
...
The per-backend delta is required only for the backend resolved from `FM_BACKEND`, then `config/backend`, then runtime auto-detection, then the tmux compatibility fallback, so a home is never told to install a tool an inactive backend or feature would need.
