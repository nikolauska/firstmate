...
make_routine_bootstrap_fixture() {
  ...
  printf '%s\n' omp > "$home/config/crew-harness"
  printf '%s\n' '{"rules":[{"when":"normal work","use":{"harness":"omp"}}],"default":{"harness":"omp","effort":"low"}}' \
...
test_crew_dispatch_active_rules_are_verbose_bootstrap_info() {
  ...
  mkdir -p "$case_dir/home/config"
  printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
  printf '%s\n' '{"rules":[{"when":"fresh news","use":{"harness":"omp"},"why":"current context"},{"when":"big feature","use":[{"harness":"omp","model":"gpt-5.6","effort":"high"},{"harness":"omp","model":"gpt-5.5","effort":"high"}]},{"when":"legacy feature","use":[{"harness":"omp"},{"harness":"omp"}],"select":"quota-balanced"}],"default":[{"harness":"omp","model":"gpt-5.6","effort":"high"},{"harness":"omp","model":"gpt-4.5","effort":"high"}]}' > "$case_dir/home/config/crew-dispatch.json"
  fakebin=$(make_fake_toolchain "$case_dir")
  add_real_jq "$fakebin"
  ...
  expect=$'BOOTSTRAP_INFO: crew dispatch active config/crew-dispatch.json\nBOOTSTRAP_INFO: crew dispatch rule: fresh news -> omp\nBOOTSTRAP_INFO: crew dispatch rule: big feature -> quota-balanced[omp/gpt-5.6/high, omp/gpt-5.5/high]\nBOOTSTRAP_INFO: crew dispatch rule: legacy feature -> quota-balanced[omp, omp]\nBOOTSTRAP_INFO: crew dispatch default: quota-balanced[omp/gpt-5.6/high, omp/gpt-4.5/high]'
...
legacy dispatch harness is flagged^{"rules":[{"when":"anything","use":{"harness":"claude"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - unverified harness: claude
unknown dispatch harness is flagged^{"rules":[{"when":"anything","use":{"harness":"spaceship"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - unverified harness: spaceship
OMP max effort is accepted^{"rules":[{"when":"OMP coding","use":{"harness":"omp","model":"openai-codex/gpt-5.6-sol","effort":"max"}}]}^empty^
OMP array use with quota-balanced is accepted^{"rules":[{"when":"big feature","use":[{"harness":"omp","model":"gpt-5.6","effort":"high"},{"harness":"omp","model":"gpt-5.5","effort":"high"}],"select":"quota-balanced"}]}^empty^
...
array profile with malformed model is flagged^{"rules":[{"when":"big feature","use":[{"harness":"omp","model":5}]}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - use profile model and effort must be non-empty strings when present
unknown select is flagged^{"rules":[{"when":"big feature","use":[{"harness":"omp"},{"harness":"omp"}],"select":"mystery"}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - unknown select: mystery
...
non-object default array entry is flagged^{"default":["omp"]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - each default profile must be an object
