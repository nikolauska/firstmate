...
Use `gh-axi` for GitHub and `chrome-devtools-axi` for browser work; load `lavish` before every Lavish operation and use it for structured decisions or reports.
...
Use plain chat for a yes-or-no decision; when several options or a structured report benefit from a visual surface, load `lavish` and follow its review workflow.
