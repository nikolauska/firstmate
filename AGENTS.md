...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then tmux; tmux is the verified reference backend (docs/tmux-backend.md), while herdr, zellij, orca, and cmux are experimental spawn backends (docs/herdr-backend.md, docs/zellij-backend.md, docs/orca-backend.md, docs/cmux-backend.md) - herdr and cmux can also be selected by runtime auto-detection, zellij and orca never are (always explicit), and codex-app is not accepted; see docs/codex-app-backend.md; inherited by secondmate homes under the primary-authoritative contract in secondmate-provisioning; section 4 owns firstmate's Herdr preference for new work
config/calm     Calm presentation preference for Pi and OMP; LOCAL, gitignored, and not inherited; see docs/configuration.md "Calm preference"
...
Before any Lavish operation, load the `lavish` skill; use `gh-axi` for GitHub, `chrome-devtools-axi` for browser work, and `lavish-axi` for structured decisions or reports; consult current help rather than memorizing flags.
...
For new work with no explicit per-task, environment, or config backend selection, dispatch with explicit `--backend herdr`; use tmux only for a concrete Herdr issue.
